...
module "config" {
  ...
  bastion_nic1_netmask    = "${cidrnetmask(data.google_compute_subnetwork.dmz.ip_cidr_range)}"
  bastion_nic1_lan_cidr   = "${data.google_compute_subnetwork.dmz.ip_cidr_range}"

  bastion_nic2_private_ip  = "${google_compute_address.bastion-engineering.address}"
  bastion_nic2_netmask     = "${cidrnetmask(data.google_compute_subnetwork.engineering.ip_cidr_range)}"
  bastion_nic2_lan_cidr    = "${var.vpc_cidr}"
  bastion_nic2_lan_netmask = "${cidrnetmask(var.vpc_cidr)}"
  bastion_nic2_lan_gateway = "${data.google_compute_subnetwork.engineering.gateway_address}"
  ...
  vpn_network_dns        = "${length(var.vpn_network_dns) == 0 ? data.google_compute_subnetwork.engineering.gateway_address: var.vpn_network_dns}"
