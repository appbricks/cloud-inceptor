...
CLOUD_INIT_SOURCES=$(sudo python -c ''"${PY_SCRIPT}"'' 2>/dev/null || \
  sudo python3 -c ''"${PY_SCRIPT}"'' 2>/dev/null) ||
...
sudo curl -sSL -o "${CLOUD_INIT_SOURCES}/DataSourceVMwareGuestInfo.py" \
...
sudo curl -sSL -o /etc/cloud/cloud.cfg.d/99-DataSourceVMwareGuestInfo.cfg \
